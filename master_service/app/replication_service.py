...
class ReplicationManager:
    ...
    async def add_message(self, message: dict):
        ...
        if success:
            logger.info(
                f'Replication complete for message {msg["id"]} (all followers confirmed)'
            )
            return msg
        else:
            logger.error(
                f'Replication FAILED for message {msg["id"]} (one or more followers down)'
            )
    ...
    async def send_message_to_follower(
        self, session: aiohttp.ClientSession, url: str, message: dict
    ):
        try:
